...
FROM docker.io/library/debian:13.3-slim@sha256:1d3c811171a08a5adaa4a163fbafd96b61b87aa871bbc7aa15431ac275d3d430 AS build
WORKDIR /build
...
RUN --mount=type=cache,target=/build,sharing=private \
	...
	--mount=type=tmpfs,target=/tmp \
	wget --quiet --no-hsts --no-clobber \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.5.4/keycloak-quarkus-dist-26.5.4.tar.gz  \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.5.4/keycloak-quarkus-dist-26.5.4.tar.gz.sha1 \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.5.4/keycloak-quarkus-dist-26.5.4.tar.gz.asc && \
	echo "$(cat keycloak-quarkus-dist-26.5.4.tar.gz.sha1) keycloak-quarkus-dist-26.5.4.tar.gz" sha1sum --quiet --check --strict - && \
	gpg --keyserver keyserver.ubuntu.com --recv-keys 861AB50E8CC6611FB6BC01A6B8F12EA26FD6EEBA && \
	gpg --verify keycloak-quarkus-dist-26.5.4.tar.gz.asc keycloak-quarkus-dist-26.5.4.tar.gz && \
	mkdir /opt/keycloak && \
	tar -xf keycloak-quarkus-dist-26.5.4.tar.gz --directory=/opt/keycloak/ --strip-components=1 --exclude=**/*.md --exclude=**/*.txt --exclude=**/*.bat
...
FROM docker.io/library/debian:13.3-slim@sha256:1d3c811171a08a5adaa4a163fbafd96b61b87aa871bbc7aa15431ac275d3d430 AS debian
COPY --link --from=java /opt/java /opt/java
...
FROM gcr.io/distroless/java17:nonroot@sha256:9d265fedb1dc3096f3ae82ee630d72c8a0199c4a13c895eae63ec70bf51c4f44 AS distroless
COPY --link --from=keycloak-runtime /opt/keycloak /opt/keycloak
ENV \
  KC_DB=postgres \
  KC_HTTP_ENABLED=true \
...
FROM docker.io/library/debian:13.3-slim@sha256:1d3c811171a08a5adaa4a163fbafd96b61b87aa871bbc7aa15431ac275d3d430 AS themes
