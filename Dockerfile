...
RUN --mount=type=cache,target=/build,sharing=private \
	...
	--mount=type=tmpfs,target=/tmp \
	wget --quiet --no-hsts --no-clobber \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.6.1/keycloak-quarkus-dist-26.6.1.tar.gz  \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.6.1/keycloak-quarkus-dist-26.6.1.tar.gz.sha1 \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.6.1/keycloak-quarkus-dist-26.6.1.tar.gz.asc && \
	echo "$(cat keycloak-quarkus-dist-26.6.1.tar.gz.sha1) keycloak-quarkus-dist-26.6.1.tar.gz" sha1sum --quiet --check --strict - && \
	gpg --keyserver keyserver.ubuntu.com --recv-keys 861AB50E8CC6611FB6BC01A6B8F12EA26FD6EEBA && \
	gpg --verify keycloak-quarkus-dist-26.6.1.tar.gz.asc keycloak-quarkus-dist-26.6.1.tar.gz && \
	mkdir /opt/keycloak && \
	tar -xf keycloak-quarkus-dist-26.6.1.tar.gz --directory=/opt/keycloak/ --strip-components=1 --exclude=**/*.md --exclude=**/*.txt --exclude=**/*.bat
...
FROM gcr.io/distroless/java17:nonroot@sha256:e07bf2a4b73e122bea42dd1eb868dae9739ac7b96e3cddacb4b067b8ba5c969e AS distroless
COPY --link --from=keycloak-runtime /opt/keycloak /opt/keycloak
ENV \
  KC_DB=postgres \
  KC_HTTP_ENABLED=true \
