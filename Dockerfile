...
RUN --mount=type=cache,target=/build,sharing=private \
	...
	--mount=type=tmpfs,target=/tmp \
	wget --quiet --no-hsts --no-clobber \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.6.4/keycloak-quarkus-dist-26.6.4.tar.gz  \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.6.4/keycloak-quarkus-dist-26.6.4.tar.gz.sha1 \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.6.4/keycloak-quarkus-dist-26.6.4.tar.gz.asc && \
	echo "$(cat keycloak-quarkus-dist-26.6.4.tar.gz.sha1) keycloak-quarkus-dist-26.6.4.tar.gz" sha1sum --quiet --check --strict - && \
	gpg --keyserver keyserver.ubuntu.com --recv-keys 861AB50E8CC6611FB6BC01A6B8F12EA26FD6EEBA && \
	gpg --verify keycloak-quarkus-dist-26.6.4.tar.gz.asc keycloak-quarkus-dist-26.6.4.tar.gz && \
	mkdir /opt/keycloak && \
	tar -xf keycloak-quarkus-dist-26.6.4.tar.gz --directory=/opt/keycloak/ --strip-components=1 --exclude=**/*.md --exclude=**/*.txt --exclude=**/*.bat
...
FROM docker.io/library/debian:13.5-slim@sha256:28de0877c2189802884ccd20f15ee41c203573bd87bb6b883f5f46362d24c5c2 AS debian
COPY --link --from=java /opt/java /opt/java
...
FROM docker.io/library/debian:13.5-slim@sha256:28de0877c2189802884ccd20f15ee41c203573bd87bb6b883f5f46362d24c5c2 AS themes
