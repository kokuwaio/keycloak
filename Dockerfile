...
FROM docker.io/library/debian:12.11-slim@sha256:2424c1850714a4d94666ec928e24d86de958646737b1d113f5b2207be44d37d8 AS build
WORKDIR /build
...
RUN --mount=type=cache,target=/build \
	...
	--mount=type=tmpfs,target=/tmp \
	wget --quiet --no-hsts --no-clobber \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.2.2/keycloak-quarkus-dist-26.2.2.tar.gz  \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.2.2/keycloak-quarkus-dist-26.2.2.tar.gz.sha1 \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.2.2/keycloak-quarkus-dist-26.2.2.tar.gz.asc && \
	echo "$(cat keycloak-quarkus-dist-26.2.2.tar.gz.sha1) keycloak-quarkus-dist-26.2.2.tar.gz" sha1sum --quiet --check --strict - && \
	gpg --keyserver keyserver.ubuntu.com --recv-keys 861AB50E8CC6611FB6BC01A6B8F12EA26FD6EEBA && \
	gpg --verify keycloak-quarkus-dist-26.2.2.tar.gz.asc keycloak-quarkus-dist-26.2.2.tar.gz && \
	mkdir /opt/keycloak && \
...
FROM gcr.io/distroless/java17:nonroot@sha256:cd7e25600feef30188400f91f0da2a03f0c0356d63b085e980ee4275e1054872 AS distroless
COPY --link --from=keycloak-runtime /opt/keycloak /opt/keycloak
ENV \
  KC_DB=postgres \
  KC_HTTP_ENABLED=true \
...
FROM docker.io/library/debian:12.11-slim@sha256:2424c1850714a4d94666ec928e24d86de958646737b1d113f5b2207be44d37d8 AS themes
