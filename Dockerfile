...
FROM docker.io/library/debian:13.3-slim@sha256:bfc1a095aef012070754f61523632d1603d7508b4d0329cd5eb36e9829501290 AS build
WORKDIR /build
...
RUN --mount=type=cache,target=/build,sharing=private \
	...
	--mount=type=tmpfs,target=/tmp \
	wget --quiet --no-hsts --no-clobber \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.5.2/keycloak-quarkus-dist-26.5.2.tar.gz  \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.5.2/keycloak-quarkus-dist-26.5.2.tar.gz.sha1 \
		https://repo1.maven.org/maven2/org/keycloak/keycloak-quarkus-dist/26.5.2/keycloak-quarkus-dist-26.5.2.tar.gz.asc && \
	echo "$(cat keycloak-quarkus-dist-26.5.2.tar.gz.sha1) keycloak-quarkus-dist-26.5.2.tar.gz" sha1sum --quiet --check --strict - && \
	gpg --keyserver keyserver.ubuntu.com --recv-keys 861AB50E8CC6611FB6BC01A6B8F12EA26FD6EEBA && \
	gpg --verify keycloak-quarkus-dist-26.5.2.tar.gz.asc keycloak-quarkus-dist-26.5.2.tar.gz && \
	mkdir /opt/keycloak && \
	tar -xf keycloak-quarkus-dist-26.5.2.tar.gz --directory=/opt/keycloak/ --strip-components=1 --exclude=**/*.md --exclude=**/*.txt --exclude=**/*.bat
...
FROM docker.io/library/debian:13.3-slim@sha256:bfc1a095aef012070754f61523632d1603d7508b4d0329cd5eb36e9829501290 AS debian
COPY --link --from=java /opt/java /opt/java
...
FROM gcr.io/distroless/java17:nonroot@sha256:2aa07e003a91925ba0e4569dd5a842d0b39b7d3adb13a621bc737b91c0a1182a AS distroless
COPY --link --from=keycloak-runtime /opt/keycloak /opt/keycloak
ENV \
  KC_DB=postgres \
  KC_HTTP_ENABLED=true \
...
FROM docker.io/library/debian:13.3-slim@sha256:bfc1a095aef012070754f61523632d1603d7508b4d0329cd5eb36e9829501290 AS themes
